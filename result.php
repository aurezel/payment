...
//        $session->logout();  // 销毁会话
    ...
    case "list":
        ...
        $data['data']['content'] = array_reverse($orders);
        $response = $data;
...
// 如果没有递订单 ID，直接回误
    ...
    case 'download':
        ...
        break;
    case 'stats':
	$stats_orders = readOrders("orders.json");
        $statistics = [];
        foreach ($stats_orders as $order) {
            // 提取日期部分 (yyyy-mm-dd)
            $date = substr($order['paymentTime'], 0, 10);
            $currency = "USD";
            if($order['currency']){
            ...
            if (!isset($statistics[$date])) {
                $statistics[$date] = [
                    ...
                    'orderCount' => 0,    // 订单数
                    'totalAmount' => 0,   // 总金额
//                    'status0Count' => 0,   // 状态为0的订单数
                    'status1Amount' => 0,   // 状态为0的订单数
                    'status1Count' => 0,   // 状态为0的订单数
                    'currency' => 'USD' //$currency
                ];
            }

            // 累加订单数
            $statistics[$date]['orderCount']++;

            if(in_array($currency,array_keys(CURRENCY_CONVERSION))){
                $statistics[$date]['totalAmount'] += (float)$order['amount']/CURRENCY_CONVERSION[$currency];
                if ($order['status'] == 1) {
                    $statistics[$date]['status1Count']++;
                    $statistics[$date]['status1Amount'] += (float)$order['amount']/CURRENCY_CONVERSION[$currency];
                }
            }else{
                $statistics[$date]['totalAmount'] += (float)$order['amount'];
                if ($order['status'] == 1) {
                    $statistics[$date]['status1Count']++;
                    $statistics[$date]['status1Amount'] += (float)$order['amount'];
                }
		
	    
	    }
//	 echo $currency.CURRENCY_CONVERSION[$currency]."|".$order['amount']."|".((float)$order['amount'] / CURRENCY_CONVERSION[$currency]).'|'.$statistics[$date]['totalAmount']."|".$statistics[$date]['createTime']."\n";
	}
	$page = count($statistics) / 20;
        $data = [];
        $data['code'] = 0;
        $data['data']['total'] = count($statistics);
        $data['data']['pages'] = ceil($page);
        $data['data']['content'] = array_reverse(array_values($statistics));
        $response = $data;
